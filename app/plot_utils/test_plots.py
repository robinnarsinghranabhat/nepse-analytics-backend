...
for sector, df in all_dfs.groupby("sector"):
    print(sector)
    df_des = (
        df[["Symbol", "stockListedShares"]]
        .drop_duplicates(subset=["Symbol"])
        .sort_values(by="stockListedShares")
        .head(10)
    )
    print(df_des)
...
def get_top_volumes(df):
    ...
    """
    sector_stats = df.groupby("Symbol").agg(
        {
            "Vol": "sum",
            "Open": "sum",
        }
...
def plot_sector(top_sector_scrips):
    ...
    for symbol, df in top_sector_scrips.groupby("Symbol"):
        ...
        fig.add_trace(
            go.Scatter(x=df["date_scraped"], y=df["Open"], mode="lines", name=symbol)
        )
    ...
    # Set layout options
    fig.update_layout(
        title="Stocks of Hydropower Sector",
        xaxis_title="Date",
...
def grouped_stock_plot(grouped_df, sectors, TOP_N=10):
    ...
    for i, sector_name in enumerate(sectors, start=1):
        ...
        least_shares = (
            df.groupby("Symbol")
            .agg({"Symbol": "first", "stockListedShares": "first"})
